...
model = genai.GenerativeModel(
    model_name="gemini-1.5-flash",  # ✅ Much higher limits
...
def test_generate():
    ...
    # ✅ SMART PROMPT (Option 3)
    basequery = (
        "Below is some text extracted from a file. If and only if the text is from a professional resume, "
        f"generate 20 interview questions for the role of '{job_title}'. "
        "If it does NOT seem to be a resume, just say: 'This is not a resume.'\n\n"
    )
    ...
    if response.text.strip().lower().startswith("this is not a resume"):
        return render_template('predict.html', cresults=[" The uploaded file doesn't look like a resume. Please upload a proper resume."])
